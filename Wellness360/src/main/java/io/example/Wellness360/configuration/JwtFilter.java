...
//To extract,validate,authenticate user if JWT token present in Authorization Header
@Component
public class JwtFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (authHeader != null && authHeader.startsWith("Bearer ")) {
			token = authHeader.substring(7); // Ignore 'Bearer ' in Authorization Header value
	...
	public JwtFilter(JwtService jwtService, ApplicationContext context, BlacklistedTokenService blacklistService) {
		super();
